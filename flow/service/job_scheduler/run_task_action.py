...
class RunTaskAction:
    ...
    def build_process_cmd_for_spark(self, task_config_json):
        ...
        # tby
        print('\n-----', task_config_json['module'])
        # print(json.dumps(task_config_json))

        # if task_config_json['module'] == 'DataIO':
        #     task_config_json['params']['label_type'] = 'float64'

        if task_config_json['module'] == 'VertSecureBoost':
            task_config_json['module'] = 'VertSmmSecureBoost'

        # if task_config_json['module'] == 'Evaluation':
        #     task_config_json['params']['eval_type'] = 'regression'
        ...
        return [
            ...
            '-r', self.task.role,
            '-m', MEMBER_ID,
            # '-c', self.task.task_conf,  # tby
            '-c', json.dumps(task_config_json),  #tby
            '-s', self.build_federation_session_id()
        ]
