...
from flow.utils.ssl_util import getGrpcCredentials
...
OVERRIDE_HOST = conf_utils.get_comm_config(consts.COMM_CONF_KEY_SSL_CRT_HOST)
...
JOB_GRPC = gateway_service_pb2_grpc.TransferServiceStub(grpc.secure_channel(
    target=f"{GATEWAY_INTRANET_HOST}:{GATEWAY_INTRANET_PORT}", 
    credentials=getGrpcCredentials(conf_utils.get_comm_config(consts.COMM_CONF_KEY_SSL_CRT_PATH)),
    options=[
        ('grpc.max_send_message_length', -1),
        ('grpc.max_receive_message_length', -1),
        ('grpc.ssl_target_name_override', OVERRIDE_HOST),
        ('grpc.default_authority', OVERRIDE_HOST)]
))
