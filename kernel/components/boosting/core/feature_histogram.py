...
class FeatureHistogram(object):
    ...
    def compute_pid_zfj(self, cur_to_split_nodes):
    ...
    def calculate_histogram(self, data_bin, grad_and_hess,
        ...
        if batch_histogram_intermediate_rs.count() == 0:  # if input sample number is 0, return empty histograms
            ...
            agg_func = self._stable_hist_aggregate if self.stable_reduce else self._hist_aggregate  # self.stable_reduce: False
    ...
    def _batch_calculate_histogram(kv_iterator, bin_split_points=None,
        ...
        for data_id, value in kv_iterator:
            ...
            # LOGGER.info("=============data_bin==========={}".format(data_bin.features.get_sparse_vector()))
        ...
        missing_bin = 1 if use_missing else 0   # false   0
        ...
        for rid in range(data_record):
            ...
            for fid, value in data_bins[rid].features.get_all_data(): # feature id , bin id
