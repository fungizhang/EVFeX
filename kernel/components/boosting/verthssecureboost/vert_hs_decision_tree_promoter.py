...
class VertDecisionTreePromoter(DecisionTree):
    ...
    def init(self, flowid, runtime_idx, data_bin, bin_split_points, bin_sparse_points, valid_features,
        ...
        self.run_cipher_compressing = cipher_compressing   # True
    ...
    def process_and_sync_grad_and_hess(self, idx=-1):
        ...
        self.transfer_inst.encrypted_grad_and_hess.remote(en_grad_hess,
                                                          ...
                                                          idx=idx)                          # 加密后的 h 和 g 转送给provider
    ...
    def sync_final_split_provider(self, dep=-1, batch=-1, idx=-1):
        ...
        final_splitinfo_provider = federation.get(name=self.transfer_inst.final_splitinfo_provider.name,
                                              ...
                                              idx=-1)
        """
        #print('################# before return final', final_splitinfo_provider)
    ...
    def fit(self):
        ...
        self.process_and_sync_grad_and_hess()              # step1 标签方先计算出每个样本的 gi 和 hi，之后将 gi和 hi 发送给各个数据提供方

        root_node = self.initialize_root_node()            # step2 标签方初始化 root 节点 计算出 root 节点的 ∑i∈I gi 和 ∑i∈I hi
        ...
        for dep in range(self.max_depth):
            LOGGER.info("start to fit depth {}, tree node queue size is {}".format(dep, len(self.tree_node_queue)))  # start to fit depth 0, tree node queue size is 1
    ...
    def compute_best_splits2(self, node_map, dep, batch_idx):
        ...
        for provider_idx, split_info_table in enumerate(provider_split_info_tables):
            ...
            ##############################
            # tby new code here (find_provider_best_split_info_hs() 重命名)
            provider_split_info, feature_best_splits_lst = self.splitter.find_provider_best_split_info_hs(split_info_table,
                                                                              self.get_provider_sitename(provider_idx),
                                                                              self.encrypter,
                                                                              cipher_decompressor=cipher_decompressor)
            provider_split_info_lst = []
            for key in provider_split_info:
                provider_split_info_lst.append(provider_split_info[key])
            
            rest_feature_splits = [i for i in feature_best_splits_lst if i not in provider_split_info_lst]
            import random
            num_items = len(rest_feature_splits)
            random_index = random.randrange(num_items)
            bind_split_info = rest_feature_splits[random_index]
            for split_info in [bind_split_info]:
                split_info.sum_grad, split_info.sum_hess, split_info.gain = None, None, None

            ##############################
            
            #print("############provider_split_info: " ,provider_split_info)
            ...
            #print("############return_split_info: " ,return_split_info)

            ######################
            # tby new code here
            return_split_info_with_bind = return_split_info + [bind_split_info]
            #print("############return_split_info_with_bind: " ,return_split_info_with_bind)
            ######################

            # 计算数据提供方的局部最优分裂结果，并将最优分裂特征的 mask _id 信息返回数据提供方。
            self.transfer_inst.federated_best_splitinfo_provider.remote(return_split_info_with_bind,                                 # return_split_info
                                                                        suffix=(dep, batch_idx), idx=provider_idx,
                                                                        role=consts.PROVIDER)
            best_splits_of_all_providers.append(split_info_list)
            #print("############best_splits_of_all_providers: " ,best_splits_of_all_providers)
        ...
        ########################
        # tby new code here
        final_provider_split_info_without_bind = []
        for provider_idx, provider_split_info in enumerate(final_provider_split_info):
            provider_split_info = provider_split_info[:-1]
            final_provider_split_info_without_bind.append(provider_split_info)
        
        ########################
        
        #print("############final_provider_split_info: " ,final_provider_split_info_without_bind)
        for masked_split_info, encoded_split_info in zip(best_splits_of_all_providers, final_provider_split_info_without_bind):  # final_provider_split_info
            for s1, s2 in zip(masked_split_info, encoded_split_info):
                s2.gain = s1.gain
                s2.sum_grad = s1.sum_grad
                s2.sum_hess = s1.sum_hess
        
        # 标签方接收到特征id后，将本地最有分裂点与数据提供方的最优分裂点比较信息增益大小，计算出全局最优分裂点
        final_best_splits = self.merge_splitinfo(best_split_info_promoter, final_provider_split_info_without_bind,                             # final_provider_split_info
                                                 need_decrypt=False)
        #print("############final_best_splits: " ,final_best_splits)
