...
class VertDecisionTreeProvider(DecisionTree):
    ...
    def convert_bin_to_real(self):
        ...
        for i in range(len(self.tree_)):
            ...
            if self.tree_[i].sitename == self.sitename:
                ...
                ###############
                print('###############fid bid i',fid, bid, i)
                if bid >= len(self.bin_split_points[fid]):
                    bid = len(self.bin_split_points[fid]) - 1
                else:
                    pass
                ###############
    ...
    #     return_lst = []
    #     nid, _ = predict_state
    #     print('######### tree len', len(tree_))
    #     if tree_[nid].sitename != sitename:
    #         return_lst.append(predict_state[0])                                                # tby
    #         #return predict_state
    #         return return_lst                                                                  # tby
    ...
    def traverse_tree(predict_state, data_inst, tree_=None,
        ...
        '''
        对于待预测对象，合作方使用己方在模型训练阶段所被选择的各个特征及特征阈值与待预测对象相应特征值进行比较,
        其中包含真实比较结果和用于盲化的虚假比较结果；
        '''
        ...
        for i in range(len(tree_)):
            if tree_[i].sitename == sitename and tree_[i].fid !=  tree_[nid].fid:
                bind_nid.append(i)
        if len(bind_nid) <= 0:
            pass
        else:
            random_bin_nid = random.sample(bind_nid,1)[0]  

            while tree_[random_bin_nid].sitename == sitename:
                fid = decoder("feature_idx", tree_[random_bin_nid].fid, split_maskdict=split_maskdict)
                bid = decoder("feature_val", tree_[random_bin_nid].bid, random_bin_nid, split_maskdict)

                if use_missing:
                    missing_dir = decoder("missing_dir", 1, random_bin_nid, missing_dir_maskdict=missing_dir_maskdict)
                else:
                    missing_dir = 1
                ...
                if use_missing and zero_as_missing:
                    ...
                    if data_inst.features.get_data(fid) == NoneType() or data_inst.features.get_data(fid, None) is None:
                        if missing_dir == 1:
                            random_bin_nid = tree_[random_bin_nid].right_nodeid
                        else:
                            random_bin_nid = tree_[random_bin_nid].left_nodeid
                    elif data_inst.features.get_data(fid) <= bid:
                        random_bin_nid = tree_[random_bin_nid].left_nodeid
                    else:
                        random_bin_nid = tree_[random_bin_nid].right_nodeid
                    ...
                    random_bin_nid = tree_[random_bin_nid].right_nodeid
            return_lst.append(random_bin_nid) 
        #return nid, 0
        return return_lst   
    ...
    def compute_best_splits2(self, node_map, dep, batch):
        ...
        if not self.complete_secure_tree:
            ...
            #print('##############provider: unmasked_split_info ',unmasked_split_info)
            # enocde 后只保留 fid的信息
            # return_split_info = self.encode_split_info(unmasked_split_info)
            
            #############
            # new code
            unmasked_split_info_part1 = unmasked_split_info[:-1]
            unmasked_split_info_part2 = unmasked_split_info[-1]
            return_split_info_part1 = self.encode_split_info(unmasked_split_info_part1)
            return_split_info_part2 = self.encode_split_info([unmasked_split_info_part2])
            return_split_info = return_split_info_part1 + return_split_info_part2
            # print('################# unmasked_split_info_part1',unmasked_split_info_part1)
            # print('################# unmasked_split_info_part1',unmasked_split_info_part2)
            # print('################# return_split_info_part1',return_split_info_part1)
            #############
            
            
            #print('##############provider: return_split_info ',return_split_info)
        ...
        for split_info in split_info_list:
            if split_info.mask_id is not None:
                try:                                                                     # new code
                    ...
                    split_info.best_fid, split_info.best_bid = -1, -1
                    masked_missing_dir = 1
