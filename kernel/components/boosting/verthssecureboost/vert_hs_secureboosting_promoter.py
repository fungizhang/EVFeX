...
from kernel.components.boosting.verthssecureboost.vert_hs_decision_tree_promoter import VertDecisionTreePromoter                               # tby 
...
class VertHsSecureBoostingPromoter(BoostingTree):
    def __init__(self):
        super(VertHsSecureBoostingPromoter, self).__init__()
    ...
    def _init_model(self, param: VertSecureBoostParam):
        ...
        super(VertHsSecureBoostingPromoter, self)._init_model(param)
    ...
    @staticmethod
    def traverse_a_tree(tree: VertDecisionTreePromoter, sample, cur_node_idx):
    ...
    @staticmethod
    def traverse_trees(node_pos, sample, trees: List[VertDecisionTreePromoter]):
        ...
        for t_idx, tree in enumerate(trees):
            ...
            rs, reach_leaf = VertHsSecureBoostingPromoter.traverse_a_tree(tree, sample, cur_node_idx)
    ...
    def merge_predict_pos(node_pos1, node_pos2):
        # 发起方与协作方的预测结果进行对比 取优
        # print('######### node_pos1',node_pos1)
        # print('######### node_pos2',node_pos2)
        ...
        '''
        接受到合作方的比较结果，发起方对来自各合作方的比较结果集合进行去盲处理，
        获得来自真实特征和特征阈值的真实比较结果
        '''
        #####################
        # new code tby 
        #print('######### node_pos1',node_pos1)
        #print('######### node_pos2',node_pos2)
        if len(node_pos2['node_pos']) ==2:
            pos_arr1 = node_pos1['node_pos']
            pos_arr2 = node_pos2['node_pos'][0]
            pos_arr3 = node_pos2['node_pos'][1]
            stack_arr = np.stack([pos_arr1, pos_arr2,pos_arr3])
        else:
            pos_arr1 = node_pos1['node_pos']
            pos_arr2 = node_pos2['node_pos']
            stack_arr = np.stack([pos_arr1, pos_arr2])
    ...
    @staticmethod
    def add_y_hat(leaf_pos, init_score, learning_rate, trees: List[VertDecisionTreePromoter], multi_class_num=None):
    ...
    @staticmethod
    def get_predict_scores(leaf_pos, learning_rate, init_score, trees: List[VertDecisionTreePromoter]
                           , multi_class_num=-1, predict_cache=None):

        if predict_cache:
            init_score = 0  # prevent init_score re-add

        predict_func = functools.partial(VertHsSecureBoostingPromoter.add_y_hat,
    ...
    def fit_a_booster(self, epoch_idx: int, booster_dim: int):
        ...
        tree = VertDecisionTreePromoter(tree_param=self.tree_param)
    ...
    def fit(self, data_inst, validate_data=None):
        ...
        for epoch_idx in range(bestIteration, self.num_trees):     # 第几个树
    ...
    def boosting_fast_predict(self, data_inst, trees: List[VertDecisionTreePromoter], predict_cache=None):
        ...
        #print("############# node_pos_tb", list(node_pos_tb.collect()))
        #print("############# final_leaf_pos", list(final_leaf_pos.collect()))
        ...
        while True:
            ...
            #print('####################sync_promoter_predict_data', list(node_pos_tb.collect()), comm_round)
    ...
    def get_model_meta(self):
        ...
        meta_name = "VertSecureBoostingTreePromoterMeta"
    ...
    def get_model_param(self):
        ...
        param_name = "VertSecureBoostingTreePromoterParam"
