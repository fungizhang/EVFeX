...
from kernel.components.boosting.verthssecureboost.vert_hs_decision_tree_provider import VertDecisionTreeProvider  # tby
...
class VertHsSecureBoostingProvider(BoostingTree):
    def __init__(self):
        super(VertHsSecureBoostingProvider, self).__init__()
    ...
    def _init_model(self, param: VertSecureBoostParam):
        ...
        super(VertHsSecureBoostingProvider, self)._init_model(param)
    ...
    def check_run_sp_opt(self):
        ...
        if self.run_sparse_opt and not self.has_transformed_data:
            ...
            sparse_to_array = functools.partial(
                VertHsSecureBoostingProvider.sparse_to_array,
    ...
    def traverse_a_tree(tree: VertDecisionTreeProvider, sample, cur_node_idx):
        ...
        # tby
        nid_lst = tree.traverse_tree(predict_state=(cur_node_idx, -1), data_inst=sample,                
                                    decoder=tree.decode, split_maskdict=tree.split_maskdict,
                                    missing_dir_maskdict=tree.missing_dir_maskdict, sitename=tree.sitename,
                                    tree_=tree.tree_, zero_as_missing=tree.zero_as_missing,
                                    use_missing=tree.use_missing)
        #print('################# nid_lst',nid_lst)
        if len(nid_lst) == 2:
            return nid_lst[0], nid_lst[1]
        else:
            return nid_lst[0], nid_lst[0]

    @staticmethod
    def traverse_trees(leaf_pos, sample, trees: List[VertDecisionTreeProvider]):

        for t_idx, tree in enumerate(trees):           # 对每一个待预测的sample 循环 num_trees 次， 更新每个tree的 node position，-1表示为leaf
            
            # print('##################t_idx , tree',t_idx)
            ...
            nid1, nid2 = VertHsSecureBoostingProvider.traverse_a_tree(tree, sample, cur_node_idx)         # tby
            # leaf_pos['node_pos'][t_idx] = nid
            
            ##########################################
            # new code tby
            # 将返回的比较结果stack在一起 最终remote给发起方
            leaf_pos['node_pos'][t_idx] = nid1
            #print('############ leaf_pos',leaf_pos)
            
            leaf_pos2 = copy.deepcopy(leaf_pos)
            leaf_pos3 = copy.deepcopy(leaf_pos)
            leaf_pos2['node_pos'][t_idx] = nid2
            #print('############ leaf_pos2',leaf_pos2)

            leaf_pos_stack = np.stack([leaf_pos['node_pos'], leaf_pos2['node_pos']])
            leaf_pos3['node_pos'] = leaf_pos_stack
            #print('############ leaf_pos3',leaf_pos3)
            ##########################################
    ...
    def boosting_fast_predict(self, data_inst, trees: List[VertDecisionTreeProvider]):
        ...
        while True:
            ...
            #print('################## promoter_node_pos', list(promoter_node_pos.collect()))
            #print('################## promoter_node_pos_len', len(list(promoter_node_pos.collect())))
            provider_node_pos = promoter_node_pos.join(data_inst, traverse_func, need_send=True)
            #print('################## provider_predict_data', list(provider_node_pos.collect()))
            #print('################## provider_predict_data_len', len(list(provider_node_pos.collect())))
