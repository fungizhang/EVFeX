...
import numpy as np  ### zfj
import time ### zfj
import math ### zfj
# import torch ### zfj
import concurrent.futures  ### zfj
import threading  ### zfj
lock = threading.Lock()  ### zfj
...
class VertDecisionTreePromoter(DecisionTree):
    def __init__(self, tree_param):
        ...
        self.sitename2 = consts.PROVIDER
    ...
    ### zfj
    def process_and_sync_grad_and_hess_zfj(self, idx=-1):
        LOGGER.info('sending Q_array to provider')
        start_time = time.time()

        if self.use_gpu:
            xgb_g_h, q_array = self.compute_gh_Q_gpu()
        else:
            xgb_g_h, q_array = self.compute_gh_Q_cpu()
        end_time = time.time()
        execution_time = end_time - start_time
        LOGGER.info("==========QR分解耗时=============={}".format(execution_time))

        q_array = q_array[:, 0:100]


        self.transfer_inst.q_array_of_promoter.remote(q_array, role=consts.PROVIDER, idx=idx)

        masked_grad_hess = self.masked_grad_hess(self.grad_and_hess)
        self.transfer_inst.encrypted_grad_and_hess.remote(masked_grad_hess,
                                                          role=consts.PROVIDER,
                                                          idx=idx)

        # masked_grad_hess_seq = []
        # sample_nums = self.grad_and_hess.count()
        # masked_grad_hess_take = self.grad_and_hess.take(sample_nums)
        # for i in range(sample_nums):
        #     masked_grad_hess_seq.append(masked_grad_hess_take[i][0])

        # self.transfer_inst.encrypted_grad_and_hess.remote(masked_grad_hess_seq,
        #                                                   role=consts.PROVIDER,
        #                                                   idx=idx)
        return xgb_g_h
    ...
    def process_and_sync_grad_and_hess(self, idx=-1):
        ...
        end_time = time.time()
        execution_time = end_time - start_time
        LOGGER.info("==========加密耗时=============={}".format(execution_time))
    ...
    def fit(self):
        ...
        # zfj: 添加条件if self.zfj_ver:
        if self.zfj_ver:
            xgb_g_h = self.process_and_sync_grad_and_hess_zfj()
        else:
        ...
        for dep in range(self.max_depth):
            ...
            for batch_idx, i in enumerate(range(0, len(self.tree_node_queue), self.max_split_nodes)):
                ...
                # zfj: 添加条件if self.zfj_ver:
                if self.zfj_ver:
                    cur_splitinfos = self.compute_best_splits2_zfj(node_map, dep, batch_idx, xgb_g_h)
                else:
                    if self.new_ver:
    ...
    ### zfj: 不完全QR分解+GPU
    def compute_gh_Q_gpu(self, M=10):
        xgb_g, xgb_h = [], []

        sample_nums = self.grad_and_hess.count()
        grad_and_hess_take = self.grad_and_hess.take(sample_nums)
        for i in range(len(grad_and_hess_take)):
            xgb_g.append(grad_and_hess_take[i][1][0])
            xgb_h.append(grad_and_hess_take[i][1][1])

        xgb_g_h = []
        xgb_g_h.append(xgb_g)
        xgb_g_h.append(xgb_h)
        xgb_g_h = np.array(xgb_g_h)

        num_cols = xgb_g_h.shape[1]
        extended_matrix = np.zeros((M, num_cols))
        extended_matrix[0:2, :] = xgb_g_h

        xgb_g_h_gpu = torch.tensor(extended_matrix, device=device)

        Q, R = torch.linalg.qr(xgb_g_h_gpu.T, mode='reduced')
        Q_cpu = Q.cpu().numpy()
        ...
        # 使用完 GPU 后，清理 GPU 占用的缓存
        torch.cuda.empty_cache()

        return xgb_g_h, Q_cpu

    ### zfj: 不完全QR分解+CPU
    def compute_gh_Q_cpu(self, M=10):
        xgb_g, xgb_h = [], []
        ...
        Q, R = np.linalg.qr(xgb_g_h.T, mode='reduced')
    ...
    def compute_gh_Q_cpu(self, M=10):
        ...
        Q, R = np.linalg.qr(xgb_g_h.T, mode='complete')


        return xgb_g_h, Q

    ### zfj: 梯度信息掩码
    def masked_grad_hess(self, grad_hess):

        gossed_num = grad_hess.count()
        ori_about_num = gossed_num * math.ceil(1 / (self.top_rate + self.other_rate * (1-self.top_rate))) * 10

        tmp_list = []
        for i in range(ori_about_num):
            tmp_list.append((i, (0.5, 0.25)))

        tmp_obj1 = session.parallelize(data=tmp_list, include_key=True, partition=1)
        result_needed = grad_hess.join(tmp_obj1, lambda x1, x2: x2)

        LOGGER.info("===========grad_hess============={}".format(grad_hess.take(100)))
        return result_needed


    ### zfj: 多次交互接收PP的数据 + 划分子矩阵 + 桶数量的异质性处理   此处有修改
    def compute_best_splits2_zfj(self, node_map, dep, batch_idx, xgb_g_h):
        ...
        acc_histograms = self.get_local_histograms(dep, self.data_bin_with_node_dispatch, self.grad_and_hess,
                                                   ...
                                                   hist_sub=True)
        # LOGGER.info("============acc_histograms=======000====={}".format(acc_histograms))
        ...
        max_trans_num_and = self.transfer_inst.max_trans_num_of_provider.get(idx=-1, suffix=(dep, batch_idx))[0]
        max_trans_num = max_trans_num_and[0]
        feature_nums = max_trans_num_and[1]
        bin_nums = max_trans_num_and[2]
        histogram_nums = len(list(node_map.keys()))
        histogram_nums_sub = math.ceil(len(list(node_map.keys())) / 2)


        ####################################### has sub
        ### 直方图相减前

        acc_histograms_sub = [[[] for _ in range(feature_nums)] for _ in range(histogram_nums_sub)]
        for i in range(histogram_nums_sub):
            for j in range(feature_nums):
                for k in range(bin_nums[j]):
                    acc_histograms_sub[i][j].append([0.0, 0.0, 0])
                acc_histograms_sub[i][j] = np.array(acc_histograms_sub[i][j])

        acc_histograms_sub = np.array(acc_histograms_sub)

        ### 直方图相减后

        acc_histograms = [[[] for _ in range(feature_nums)] for _ in range(histogram_nums)]
        for i in range(histogram_nums):
            for j in range(feature_nums):
                for k in range(bin_nums[j]):
                    acc_histograms[i][j].append([0.0, 0.0, 0])

        acc_histograms = np.array(acc_histograms)


        gh_aggre = [[[] for _ in range(feature_nums)] for _ in range(histogram_nums_sub)]
        for i in range(histogram_nums_sub):
            for j in range(feature_nums):
                for k in range(bin_nums[j]):
                    gh_aggre[i][j].append([0.0, 0.0])


        pid_map = self.transfer_inst.pid_map_of_provider.get(idx=-1, suffix=(dep, batch_idx))[0]

        #######################################   并行版本
        def process_received_result(acc_histograms_pre):
            W = acc_histograms_pre[0][0]
            trans_num = acc_histograms_pre[0][1]
            sub_R = acc_histograms_pre[0][2]

            # W_gpu = torch.tensor(W, device=device)

            if (trans_num + 1) * sub_R > sample_nums:
                xgb_g_h_sub = xgb_g_h[:, trans_num * sub_R: sample_nums]
            else:
                xgb_g_h_sub = xgb_g_h[:, trans_num * sub_R: (trans_num + 1) * sub_R]

            # 计算局部的gh_aggre
            local_gh_aggre = [[np.zeros_like(gh_aggre[h][f]) for f in range(feature_nums)] for h in
                              range(histogram_nums_sub)]
            for h in range(histogram_nums_sub):
                for f in range(feature_nums):
                    local_gh_aggre[h][f] = np.dot(xgb_g_h_sub, W[h][f]).T

            # 通过线程锁更新全局的gh_aggre
            with lock:
                for h in range(histogram_nums_sub):
                    for f in range(feature_nums):
                        gh_aggre[h][f] = np.add(np.array(gh_aggre[h][f]), local_gh_aggre[h][f])


        # 使用ThreadPoolExecutor实现接收方并行处理接收到的计算结果
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = []
            while acc_trans < max_trans_num:
                # start_time1 = time.time()
                acc_histograms_pre = self.transfer_inst.w_array_of_provider.get(idx=-1, suffix=(dep, batch_idx, acc_trans))
                # end_time1 = time.time()
                # execution_time = end_time1 - start_time1
                # LOGGER.info("==========111============{}".format(execution_time))

                futures.append(executor.submit(process_received_result, acc_histograms_pre))
                acc_trans += 1

            concurrent.futures.wait(futures)

        acc_data_num = self.transfer_inst.acc_data_num_of_provider.get(idx=-1, suffix=(dep, batch_idx))
        acc_data_num = acc_data_num[0][0]

        # LOGGER.info("============acc_data_num=======111====={}".format(acc_data_num))

        for h in range(histogram_nums_sub):
            for f in range(feature_nums):
                acc_histograms_sub[h][f][:, 0] = gh_aggre[h][f][:, 0]
                acc_histograms_sub[h][f][:, 1] = gh_aggre[h][f][:, 1]
                acc_histograms_sub[h][f][:, 2] = acc_data_num[h][f]

        # LOGGER.info("============pid_map============{}".format(pid_map))
        ### 存储接收到的直方图，并利用直方图相减计算新的直方图，并保存新的直方图，并整理出这一层所有在map里的节点的直方图汇总
        if pid_map == 1:
            self.histogram_saved[0] = acc_histograms_sub[0]
            acc_histograms[0] = self.histogram_saved[0]


        else:
            node_id_list = list(pid_map.keys())
            for i in range(len(node_id_list)):
                if i % 2 == 0:
                    node_id = node_id_list[i]
                    hist_id = math.floor(i / 2)
                    self.histogram_saved[node_id] = acc_histograms_sub[hist_id]
                else:
                    node_id = node_id_list[i]
                    p_id = pid_map[node_id]
                    self.histogram_saved[node_id] = np.subtract(self.histogram_saved[p_id],
                                                                self.histogram_saved[node_id - 1])

            for i in range(histogram_nums):
                acc_histograms[i] = self.histogram_saved[node_id_list[i]]

        acc_histograms = acc_histograms.tolist()
        # LOGGER.info("============acc_histograms=======111====={}".format(np.array(acc_histograms).shape))
        # LOGGER.info("============acc_histograms=======111====={}".format(acc_histograms))


        #####  计算直方图之后，构造出DSource数据，后续流程同天冕
        tmp1 = []
        map_keys = list(node_map)
        for i in range(histogram_nums):
            for j in range(feature_nums):
                tmp1.append(((map_keys[i], j), (j, acc_histograms[i][j])))

        tmp_obj = session.parallelize(data=tmp1, include_key=True, partition=1)

        self.transfer_inst.dsource_of_promoter.remote(tmp_obj, suffix=(dep, batch_idx), idx=-1, role=consts.PROVIDER)
