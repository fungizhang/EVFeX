...
class VertDecisionTreeProvider(DecisionTree):
    ...
    ### zfj: 接收Q矩阵和masked梯度
    def sync_encrypted_grad_and_hess_zfj(self):
        LOGGER.info("get encrypted grad and hess")
        self.q_array = self.transfer_inst.q_array_of_promoter.get(idx=0)

        self.grad_and_hess = self.transfer_inst.encrypted_grad_and_hess.get(idx=0)


        # masked_grad_hess_seq = self.transfer_inst.encrypted_grad_and_hess.get(idx=0)

        # tmp_list = []
        # for i in masked_grad_hess_seq:
        #     tmp_list.append((i, (999, 999)))
        #
        # self.grad_and_hess = session.parallelize(data=tmp_list, include_key=True, partition=1)
    ...
    def update_instances_node_positions(self):
        ...
        self.data_bin_with_position = self.data_bin_with_position.join(self.grad_and_hess, lambda x1, x2: x1)
    ...
    ### zfj：多次交互   此处有修改
    def compute_best_splits2_zfj(self, node_map, dep, batch):
        ...
        if not self.complete_secure_tree:
            ...
            sample_nums = data.count()
            if self.run_sparse_opt:
                data = self.data_bin_dense_with_position

            feature_nums = data.take(sample_nums)[0][1][0].features.shape
            bin_nums = []
            for i in range(feature_nums):
                bin_nums.append(self.bin_split_points[i].shape[0])

            ######################### has sub
            histogram_nums_pre = len(list(node_map.keys()))
            histogram_nums = math.ceil(len(list(node_map.keys())) / 2)

            data_bin_num_list = [[[] for _ in range(feature_nums)] for _ in range(histogram_nums)]
            for i in range(histogram_nums):
                for j in range(feature_nums):
                    for k in range(bin_nums[j]):
                        data_bin_num_list[i][j].append([])


            data_pos_sample_nums = data.take(sample_nums)

            # LOGGER.info("============data_pos_sample_nums============{}".format(data_pos_sample_nums))

            for feature in range(feature_nums):
                for iid in range(sample_nums):
                    sample_node_id = data_pos_sample_nums[iid][1][1][1]
                    if sample_node_id in list(node_map.keys()):
                        node_pos = node_map[sample_node_id]
                        if node_pos % 2 == 1:
                            continue
                        node_pos_screen = math.floor(node_pos / 2)
                        if feature in data_pos_sample_nums[iid][1][0].features.get_sparse_vector():
                            bin_id = data_pos_sample_nums[iid][1][0].features.get_sparse_vector()[feature]
                            data_bin_num_list[node_pos_screen][feature][bin_id].append(iid)


            # sub_R = int(sample_nums * 0.05)  # 划分子矩阵时，设置为sub_R行一个子矩阵，后续也可改为一个全局参数传入
            # sub_R = 1000 if int(sample_nums < 1000) else int(sample_nums * 0.05)
            sub_R = 10000000
            max_trans_num = math.ceil(sample_nums / sub_R)
            max_trans_num_and = [max_trans_num, feature_nums, bin_nums]
            self.transfer_inst.max_trans_num_of_provider.remote(max_trans_num_and, role=consts.PROMOTER, idx=-1,
                                                                suffix=(dep, batch))

            pid_map = self.get_pid_zfj(self.cur_split_nodes)
            if pid_map:
                self.transfer_inst.pid_map_of_provider.remote(pid_map, role=consts.PROMOTER, idx=-1,
                                                              suffix=(dep, batch))
            else:
                self.transfer_inst.pid_map_of_provider.remote(1, role=consts.PROMOTER, idx=-1,
                                                              suffix=(dep, batch))
            ...
            # ###########################  并行处理
            acc_histograms_pre_list = []

            def process_trans_num(trans_num):
                acc_histograms_pre = self.get_local_histograms_zfj(trans_num, Z, sub_R, bin_nums, feature_nums,
                                                                   histogram_nums, data_bin_num_list)
                acc_histograms_pre_list.append(acc_histograms_pre)


            with concurrent.futures.ThreadPoolExecutor() as executor:
                while trans_num < max_trans_num:
                    # 提交单个trans_num的计算和传送任务给线程池
                    executor.submit(process_trans_num, trans_num)
                    trans_num += 1


            start_time = time.time()
            for i in range(max_trans_num):
                self.transfer_inst.w_array_of_provider.remote(acc_histograms_pre_list[i], role=consts.PROMOTER, idx=-1,
                                                          suffix=(dep, batch, i))
            end_time = time.time()
            execution_time = end_time - start_time
            LOGGER.info("==========4444444444444444=============={}".format(execution_time))

            acc_data_num = self.get_acc_data_num_zfj(bin_nums, feature_nums, histogram_nums, data_bin_num_list)
            self.transfer_inst.acc_data_num_of_provider.remote(acc_data_num, role=consts.PROMOTER, idx=-1,
                                                               suffix=(dep, batch))


            acc_histograms = self.transfer_inst.dsource_of_promoter.get(suffix=(dep, batch), idx=0)
    ...
    #
    #         acc_histograms = self.get_local_histograms(dep, data, self.grad_and_hess, node_sample_count,
    #                                                    self.cur_split_nodes, node_map, ret='tensor',
    #                                                    sparse_opt=self.run_sparse_opt, hist_sub=True,
    #                                                    bin_num=self.bin_num)
    #         LOGGER.info("============acc_histograms============{}".format(acc_histograms))
    #         # asd = [acc_histograms, self.valid_features, self.sitename]
    #         best_split_info_provider = self.splitter.find_split(acc_histograms, self.valid_features,
    #                                                             self.data_bin._partitions, self.sitename,
    #                                                             self.use_missing, self.zero_as_missing)
    #         # LOGGER.info("============best_split_info_provider============{}".format(best_split_info_provider))
    #         unmasked_split_info = self.unmask_split_info(best_split_info_provider, self.inverse_fid_bid_random_mapping,
    #                                                      self.missing_dir_mask_left[dep],
    #                                                      self.missing_dir_mask_right[dep])
    #         # LOGGER.info("==============unmasked_split_info=========={}".format(unmasked_split_info))
    #         return_split_info = self.encode_split_info(unmasked_split_info)
    #         # LOGGER.info("==============return_split_info=========={}".format(return_split_info))
    #         self.transfer_inst.encrypted_splitinfo_provider.remote(return_split_info,
    #                                                                role=consts.PROMOTER,
    #                                                                idx=-1,
    #                                                                suffix=(dep, batch))
    #
    ...
    def fit(self):
        ...
        if self.zfj_ver:
            self.sync_encrypted_grad_and_hess_zfj()
            ### now, the self.grad_and_hess that the provider receives is array Q
        else:
        ...
        for dep in range(self.max_depth):
            ...
            if self.zfj_ver:
                #####  zfj: 修改self.node_dispatch 避免统计样本量减小
                get_node_dispatch = self.sync_node_positions(dep)

                if self.node_dispatch == None:
                    self.node_dispatch = get_node_dispatch
                else:
                    self.node_dispatch = self.node_dispatch.union(get_node_dispatch, lambda x1, x2: x2)
            else:
                self.node_dispatch = self.sync_node_positions(dep)
            ...
            for i in range(0, len(self.tree_node_queue), self.max_split_nodes):
                ...
                if self.zfj_ver:
                    start_time = time.time()
                    self.compute_best_splits2_zfj(node_map=self.get_node_map(self.cur_split_nodes), dep=dep, batch=batch)
                    end_time = time.time()
                    execution_time = end_time - start_time
                    LOGGER.info("==========AAAAAAAAAAAAAAAAAA=============={}".format(execution_time))
                else:
                    if self.new_ver:
                        self.compute_best_splits2(node_map=self.get_node_map(self.cur_split_nodes), dep=dep, batch=batch)
                    else:
        ...
        LOGGER.info("==========BBBBBBBBBBBBBBBBB=============={}".format(execution_time))
