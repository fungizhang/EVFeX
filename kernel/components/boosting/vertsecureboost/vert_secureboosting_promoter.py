...
from kernel.components.boosting import VertDecisionTreePromoter
...
class VertSecureBoostingPromoter(BoostingTree):
    def __init__(self):
        super(VertSecureBoostingPromoter, self).__init__()
    ...
    def compute_loss(self):
        ...
        ##### zfj123
        # LOGGER.info("===========样本的权重========={}".format(self.F.take(100000)))
        LOGGER.info("===========float(loss)============={}".format(float(loss)))
        return float(loss)
    ...
    def get_predict_scores(leaf_pos, learning_rate, init_score, trees: List[VertDecisionTreePromoter]
        ...
        predict_func = functools.partial(VertSecureBoostingPromoter.add_y_hat,
