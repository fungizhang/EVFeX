...
from kernel.components.linr.horzlinr.horz_linr_base import HorzLinRBaseModel                            # tby
from kernel.components.linr.horzlinr.horz_linr_gradient import LinearGradient                           # tby
from kernel.components.linr.linr_model_weight import LinRModelWeights as LinearRegressionWeights        # tby
...
class HorzLinRClient(HorzLinRBaseModel):                                                                # tby
    def __init__(self):
        super(HorzLinRClient, self).__init__()
        self.gradient_operator = LinearGradient()                                                       # tby
        self.loss_history = []
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        while self.n_iter_ < max_iter + 1:
            ...
            if (self.n_iter_ > 0 and self.n_iter_ % self.aggregate_iters == 0) or self.n_iter_ == max_iter:
                ...
                self.model_weights = LinearRegressionWeights(weight.unboxed, self.fit_intercept)                              #tby
    ...
    def predict(self, data_instances):
        ...
        #                                                                {"1": x[0], "0": 1 - x[0]}])                  # 需要改
        predict_y = self.compute_wx(data_instances, self.model_weights.coef_, self.model_weights.intercept_)
     
        predict_result = data_instances.join(predict_y, lambda d, pred: [d.label, pred,
                                                                                 pred, {"label": pred}])
