...
class LinearParam(BaseParam):
    """
    Parameters used for Linear Regression both for Horz mode or Vert mode.

    Parameters
    ----------
    penalty : str, 'L1', 'L2' or None. default: 'L2'
        Penalty method used in LinR. Please note that, when using encrypted version in HorzLinR,
        'L1' is not supported.
    ...
    def check(self):
        ...
        if self.penalty is None:
            ...
            if self.penalty not in [consts.L1_PENALTY, consts.L2_PENALTY, 'NONE']:
                raise ValueError(
                    descr + "penalty not supported, penalty should be 'L1', 'L2' or 'none'".format(self.penalty))
        ...
        if type(self.alpha).__name__ not in ["float", 'int']:
            raise ValueError(
                descr + "alpha {} not supported, should be float or int type".format(self.alpha))
        ...
        if not isinstance(self.learning_rate, (float, int)):
            raise ValueError(
                descr + "learning_rate {} not supported, should be float or int type".format(
                    self.learning_rate))
        ...
        if type(self.early_stop).__name__ != "str":
            ...
            if self.early_stop not in ['diff', 'abs', 'weight_diff']:
                raise ValueError(
                    descr + "early_stop not supported, converge_func should be 'diff', 'weight_diff' or 'abs'")
        ...
        if self.encrypt_param.method not in [consts.PAILLIER, None]:
            raise ValueError(
                descr + "encrypted method support 'Paillier' or None only")
        ...
        if type(self.decay_sqrt).__name__ not in ['bool']:
            raise ValueError(
                descr + "decay_sqrt {} not supported, should be 'bool'".format(self.decay_sqrt))
...
class HorzLinearParam(LinearParam):
    """
    Parameters
    ----------
    ...
    def __init__(self, penalty='L2',
                 ...
                 cv_param=CrossValidationParam(),                                          #tby  mode=consts.HORZ,need_cv=True
                 decay=1, decay_sqrt=True,
                 aggregate_iters=1, validation_freqs=2,                                                     #tby freqs写死
                 early_stopping_rounds=None,
                 metrics=None,
                 use_first_metric_only=False
                 ):
        super(HorzLinearParam, self).__init__(penalty=penalty, tol=tol, alpha=alpha, optimizer=optimizer,
...
class VertLinearParam(LinearParam):
    ...
    def __init__(self, penalty='L2',
        ...
        super(VertLinearParam, self).__init__(penalty=penalty, tol=tol, alpha=alpha, optimizer=optimizer,
                                                batch_size=batch_size,
                                                learning_rate=learning_rate,
                                                init_param=init_param, max_iter=max_iter, early_stop=early_stop,
                                                cv_param=cv_param,
