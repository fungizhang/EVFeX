...
from kernel.components.linr.vertlinr.sync import loss_sync
...
class Promoter(VertGradientBase, loss_sync.Promoter):
    ...
    def compute_fore_gradient(self, data_instances, model_weights, offset=None):
        ...
        """
        # X.dot(W)+b promoter这一方的forwards
        ...
        # 结合 promoter 和 provider的 forwards
        ...
        y_hat = self.forwards    #tby 线性回归去掉 sigmoid()
    ...
    def federated_compute_gradient_and_loss(self, data_instances, cipher_operator, encrypted_calculator, model_weights,
                                            optimizer,loss_method, n_iter_, batch_index, offset=None):
        """
        ...
        encrypted_fore_gradient = encrypted_calculator[batch_index].encrypt(self.fore_gradient)  #加密
        self.remote_fore_gradient(encrypted_fore_gradient, suffix=current_suffix)                #将加密的d 发给provider
...
class Provider(VertGradientBase, loss_sync.Provider):
    ...
    def federated_compute_gradient_and_loss(self, data_instances, cipher_operator, encrypted_calculator,
        ...
        r = RandomNumberGenerator(-1, 1).generate_random_number(unilateral_gradient.shape)  # add random r shape as same as gradient
        ...
        gradient_r = self.sync_gradient_r(en_gradient_r, suffix=current_suffix)  # provider_en_gradient_r_transfer 同步给promoter
        ...
        self.remote_loss_regular(loss_regular, suffix=current_suffix)  #同步loss给promoter
