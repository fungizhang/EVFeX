...
from kernel.components.linr.vertlinr.sync import converg_sync                       # tby                     
from kernel.components.linr.vertlinr.sync import iter_sync, paillier_keygen_sync, batch_info_sync  # tby
from kernel.components.linr.vertlinr.sync import vert_linr_gradient_and_loss   # tby
 
from kernel.components.linr.vertlinr.vert_linr_base import VertLinRBaseModel # tby
...
class VertLinRPromoter(VertLinRBaseModel):
    def __init__(self):
        super().__init__()
    ...
    def fit(self, data_instances, validate_data=None):
        """
        Train linear model of role promoter
        Parameters
        ----------
        data_instances: DSource of Instance, input data
        """

        LOGGER.info("Enter vert_linr_promoter fit")
    ...
    def fit_binary(self, data_instances, validate_data=None):
        LOGGER.info("Enter vert_linr_promoter fit_binary")  #tby
        self.header = self.get_header(data_instances)
        ...
        self.model_weights = LinRModelWeights(w, fit_intercept=self.fit_intercept)
        ...
        while self.n_iter_ < self.max_iter:
            ...
            for batch_data in batch_data_generator:
                ...
                gradient, loss_list = self.gradient_loss_operator. \
                    federated_compute_gradient_and_loss(                      # federated_compute_gradient_and_loss() need be recode  ##tby
                    batch_data,
                ...
                self.model_weights = self.optimizer.update_model(self.model_weights, gradient)  # code
                batch_index += 1
            ...
            if self.model_param.early_stop == 'weight_diff':
                ...
                weight_diff = base_operator.norm(total_gradient)
            ...
            if self.validation_strategy:
                LOGGER.debug('LinR promoter running validation')          #tby
            ...
            # export model
            self.tracker.save_training_best_model(self.export_model())   # self._get_param()
        ...
        for pw in provider_weights:
            ...
            provider_weight_dict = self.parse_param(pw[0], provider_header)   # parse_param
    ...
    def predict(self, data_instances):                               
        ...
        provider_preds= self.transfer_variable.provider_partial_prediction.get(idx=-1)
        

        LOGGER.info("Get prediction from Provider")

        # promoter probability
        for provider_pred in provider_preds:
            pred = pred.join(provider_pred, lambda g, h: g + h)
