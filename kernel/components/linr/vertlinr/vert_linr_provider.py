...
from kernel.components.linr.vertlinr.sync import iter_sync, paillier_keygen_sync, batch_info_sync, converg_sync #tby
...
class VertLinRProvider(VertLinRBaseModel):
    def __init__(self):
        super(VertLinRProvider, self).__init__()
        self.batch_num = None
        ...
        self.gradient_loss_operator = vert_linr_gradient_and_loss.Provider()
    ...
    def fit(self, data_instances, validate_data=None):
        """
        Train linear model of role provider
        Parameters
        ----------
        data_instances: DSource of Instance, input data
        """
        LOGGER.info("Enter vert_linr provider fit")

        self.header = self.get_header(data_instances) 

        # classes = self.one_vs_rest_obj.get_data_classes(data_instances) #tby
    ...
    def fit_binary(self, data_instances, validate_data):
        LOGGER.info("Enter vert_linr provider fit_binary")
        ...
        self.model_weights = LinRModelWeights(w, fit_intercept=self.init_param_obj.fit_intercept)
        ...
        while self.n_iter_ < self.max_iter:
            ...
            if self.validation_strategy:
                LOGGER.debug('LinR provider running validation')
    ...
    def predict(self, data_instances):
        ...
        self.transfer_variable.provider_partial_prediction.disable_auto_clean()
        ...
        pred_provider = self.compute_wx(data_instances, self.model_weights.coef_, self.model_weights.intercept_)
        self.transfer_variable.provider_partial_prediction.remote(pred_provider, role=consts.PROMOTER, idx=0)
        LOGGER.info("Remote partial prediction to Promoter")
