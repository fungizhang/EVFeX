...
import time  # zfj
import sys
sys.path.append(r'/data/zhangfangjiao/codes/WeFe-main')
# sys.path.append(r'C:\Users\zhangfangjiao\Desktop\WeFe-main')
...
from kernel.examples.handler.component import VertSmmSecureBoost
...
def main(config="../../config.yaml", param="./binary_config.yaml", namespace="wefe_data"):
    ...
    # init handler
    handler = Handler(job_id="job_vert-smm-sbt_new"+ str(random.randint(0, 999999999999)), backend=backend, work_mode=work_mode, db_type=db_type,
                      fl_type='vertical') \
    ...
    dataio_0.get_member_instance(role="promoter", member_id=promoter).component_param(table=promoter_train_data,with_label=True,output_format="dense")
    dataio_0.get_member_instance(role="provider", member_id=provider).component_param(table=provider_train_data,with_label=False)
    ...
    vert_smm_secure_boost_0 = VertSmmSecureBoost(name="vert_smm_secure_boost_0",
    ...
    # evaluation_1 = Evaluation(name="evaluation_1", eval_type=param['eval_type'])
    ...
    handler.add_component(vert_smm_secure_boost_0, data=Data(train_data=intersect_0.name))
    handler.add_component(evaluation_0, data=Data(data=vert_smm_secure_boost_0.name))
    # handler.add_component(evaluation_1, data=Data(data=vert_secure_boost_0.name))
    ...
    print(handler.get_metric_summary(name='evaluation_0', component_name="vert_smm_secure_boost_0"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("VERT SMM SBT JOB")
    parser.add_argument("-config", type=str,
                        help="config file",default='/data/zhangfangjiao/codes/WeFe-main/kernel/examples/config.yaml')
    parser.add_argument("-param", type=str,
                        help="config file for params",default='/data/zhangfangjiao/codes/WeFe-main/kernel/examples/demo/vert_smm_sbt/binary_config.yaml')
    ...
    print("========总时长======={}".format(execution_time))
