...
from common.python.utils.clean import Cleaner
...
class Variable(object):
    ...
    def remote_parties(self, obj, parties: Union[list, Member], suffix=tuple()):
    ...
    def get_parties(self, parties: Union[list, Member], suffix=tuple()):
        if not isinstance(suffix, tuple):
            suffix = (suffix,)
        tag = self.generate_tag(*suffix)
    ...
    def remote(self, obj, role=None, idx=-1, suffix=tuple(), member_id_list=None):
        ...
        Args:
            ...
            role: role of parties to sent to, use one of ['Provider', 'Promoter', 'Arbiter', None].
        ...
        if not member_id_list:
            ...
            for role in role:
                role_parties = [Member(role, member_id) for member_id in member_id_list]
                parties.extend(role_parties)
        if idx >= 0:
            parties = parties[idx]
        return self.remote_parties(obj=obj, parties=parties, suffix=suffix)

    def get(self, idx=-1, suffix=tuple(), member_id_list=None):
        """
        ...
        if not member_id_list:
            ...
            for role in src_role:
                role_parties = [Member(role, member_id) for member_id in member_id_list]
                src_parties.extend(role_parties)
